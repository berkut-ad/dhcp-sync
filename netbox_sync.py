...
def sync_lease(lease, netbox_url, token):
    ...
    try:
        r = requests.get(f"{base_url}?address={ip}/32", headers=headers)
        r.raise_for_status()
        ...
        if results:
            ...
            r = requests.patch(update_url, headers=headers, data=json.dumps(payload))
...
def expire_lease(ip, netbox_url, token):
    ...
    try:
        ...
        update_url = f"{base_url}{ip_id}/"
        r = requests.patch(update_url, headers=headers, data=json.dumps(update_payload))
